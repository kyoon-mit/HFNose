...
void EnergyResolution::analyze ( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
    ...
    const CaloSubdetectorGeometry* geom_HGCal_ = handle_HGCalGeometry.product();
        ...
        {       
            ...
            { // Nested to save a bit of calculation
                     ...
                     ( std::abs(caloTruth.eta()) < select_EtaHigh_ ) )
                {
                    ...
                    if ( caloTruth_count_validation > 1 ) return;
    ...
    if ( caloTruth_count_validation == 0 ) return;
...
//            Int_t hit_DetLayer = hit_DetId.layer();
            ...
            GlobalPoint hit_globalPosition = (geom_HGCal_)->getGeometry(hit_DetId)->getPosition(hit_id);
...
void EnergyResolution::beginJob ()
{
    ...
    histContainer_["EDist"] = fs->make<TH1F>("EDist", "Energy Distribution", 20, 0, 1000);
...
void EnergyResolution::endJob ()
{
    ...
    // Should not do this calc from a binned hist, but from values themselves
    // Alt, you can use RooFit (seems like the best option to me)
    ...
    const Float_t E_Res             = det_E_StdDev / det_E_StdDev;
