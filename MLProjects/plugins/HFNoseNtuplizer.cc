...
HFNoseNtuplizer::HFNoseNtuplizer(const edm::ParameterSet& iConfig) :
    token_CaloParticles_( consumes<std::vector<CaloParticle>>( edm::InputTag ("mix", "MergedCaloTruth", "HLT") ) ),
    token_HFNoseRecHits_( consumes<HGCRecHitCollection>( edm::InputTag("HGCalRecHit", "HGCHFNoseRecHits", "RECO") ) ),
    token_Tracksters_( consumes<std::vector<ticl::Trackster>>( iConfig.getUntrackedParameter<edm::InputTag> ("InputTag_Tracksters", edm::InputTag("ticlTrackstersHFNoseEM") ) ) ),
    token_HFNoseLayerClusters_( consumes<reco::CaloClusterCollection>( edm::InputTag("hgcalLayerClustersHFNose") ) ),
    withPU_( iConfig.getUntrackedParameter<bool> ("withPU", false ) )
{
...
{
    //  std::cout << " IN getTracksterEnergyFromCP \n";
    ...
    for ( unsigned int ilc = 0; ilc < lcIdxs_.size(); ++ilc )
    {
        ...
        const reco::CaloCluster& lc = lcs[lcIdxs_[ilc]];
...
}  // end of getTracksterEnergyFromCP
                                                                            ...
                                                                            const reco::CaloClusterCollection & lcs )
{
    ...
    for ( unsigned int ilc = 0; ilc < lcIdxs_.size(); ++ilc )
    {
        const reco::CaloCluster & lc = lcs.at(lcIdxs_[ilc]);
        auto const & hf = lc.hitsAndFractions();

        int layer_ = rhtools_.getLayerWithOffset(hf[0].first);
...
//    iSetup.get<IdealGeometryRecord>().get( "HGCalHFNoseSensitive", handle_HGCalGeometry ); 
    ...
    for (const auto& it_cp : cps)
    {
        ...
        if ( (abs(cp.eta()) < 3.0) || (abs(cp.eta()) > 4.2) ) continue;
    ...
    for (unsigned int icp = 0; icp < caloparticles.size(); ++icp)
    {
        ...
        {
            float tracksterEnFromCP_ = getTracksterEnergyFromCP(tracksters[itrackster], lcs, caloparticles[icp]);
